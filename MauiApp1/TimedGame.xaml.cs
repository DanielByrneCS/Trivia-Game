...
{
    public TimedGame(int difficulty, int type)
    {
        InitializeComponent();
    ...
    private void AnswerClicked(object sender, EventArgs e)
    {
        ...
        if (button.Text.Equals(questionResponse.results[currentQuestion].correct_answer))
        {
            ...
            else
            {
                ...
                QuestionsCorrect++;
                questionTitle.Text = "";
                GameEnd(QuestionsCorrect, QuestionsIncorrect, Preferences.Get("TimerLength", 60));
    ...
    private async void GameEnd(int questionsCorrect, int questionsIncorrect, int timerLength)
    {
        await Navigation.PushAsync(new ResultsPage(questionsCorrect, questionsIncorrect, Difficulty, GameType, questionType, timerLength, 1));
    }
